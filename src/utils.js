...
function getDanmakuWins() {
  return BrowserWindow.getAllWindows().filter((win) =>
    win.getTitle().startsWith("Danmaku"),
  );
}
...
async function setSys(key, value) {
  if (value !== undefined) {
    await getUserDB().run(
      `insert or replace into sys values (?, ?)`,
      key,
...
async function consultDictionary(word) {
  ...
  const res = await getDictDB().get(
    `select *, ${dict.field} as paraphrase from ${dict.table} where word = ?`,
    word,
  );
  return res;
}

async function consultRichDictionary(word) {
  const rdb = getRichDictDB();
  if (!rdb) return null;

  // 1. Query word master + book name
  const wordEntry = await rdb.get(
    `SELECT wm.*, wb.name as book_name
     FROM word_master wm
     LEFT JOIN word_books wb ON wm.book_slug = wb.slug
     WHERE wm.word = ?`,
    word,
  );
  if (!wordEntry) return null;

  // 2. Query related data in parallel, max 5 per category
  const slug = wordEntry.slug;
  const [translations, phrases, sentences, synonyms, cognates] =
    await Promise.all([
      rdb.all(
        `SELECT pos, trans_cn, trans_en FROM word_translations WHERE word_slug = ? LIMIT 5`,
        slug,
      ),
      rdb.all(
        `SELECT content, trans_cn FROM word_phrases WHERE word_slug = ? LIMIT 5`,
        slug,
      ),
      rdb.all(
        `SELECT content, trans_cn FROM word_sentences WHERE word_slug = ? LIMIT 5`,
        slug,
      ),
      rdb.all(
        `SELECT pos, content, trans_cn FROM word_synonyms WHERE word_slug = ? LIMIT 5`,
        slug,
      ),
      rdb.all(
        `SELECT pos, content, trans_cn FROM word_cognates WHERE word_slug = ? LIMIT 5`,
        slug,
      ),
    ]);

  // 3. Return clean JSON (field rename, strip internal IDs)
  return {
    word: wordEntry.word,
    bookName: wordEntry.book_name || "",
    pronounce: {
      us: wordEntry.us_pronounce || "",
      uk: wordEntry.uk_pronounce || "",
    },
    ...
    phrases: phrases.map((p) => ({ content: p.content, cn: p.trans_cn })),
    sentences: sentences.map((s) => ({ content: s.content, cn: s.trans_cn })),
    synonyms: synonyms.map((s) => ({
      pos: s.pos,
      content: s.content,
      cn: s.trans_cn,
    })),
    cognates: cognates.map((c) => ({
      pos: c.pos,
      content: c.content,
      cn: c.trans_cn,
    })),
