...
async function setDanmakuLauncher(dwords) {
  ...
  dwords.danmakuLauncher = setInterval(
...
async function launchDanmaku(dwords) {
  ...
  const word = await getUserDB().get(
    `with u as (
    ...
    planID,
    maxCurrent,
    ...dwords.currentDanmakus
  );
  if (!word) return;

  // Increment appear_count
  await getUserDB().run(
    `UPDATE words SET appear_count = appear_count + 1 WHERE plan_id = ? AND word = ?`,
    planID,
    word.word
  );
  word.appear_count = (word.appear_count || 0) + 1;

  let res = await consultDictionary(word.word);
  if (!res || !res.phonetic) {
    const rich = await consultRichDictionary(word.word);
    if (rich && rich.pronounce) {
      if (!res) res = {};
      res.phonetic = rich.pronounce.us || rich.pronounce.uk || "";
    }
  }
  if (res) {
...
async function setDanmakuMover(dwords) {
  ...
  const { danmakuSpeed, displayArea } = await getSettings(
    "danmakuSpeed",
    "displayArea"
  );
