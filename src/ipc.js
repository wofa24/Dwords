...
const { getUserDB, getDictDB, getRichDictDB } = require("./database");
const {
  getWinByWebContentsID,
  getMainWin,
  getSys,
  setSys,
  genUUID,
  parseCSV,
  consultDictionary: consult,
  consultRichDictionary,
  findWinByTitle,
} = require("./utils");
...
const { analyzeWord } = require("./word_analysis");
...
async function getWords(_, planID, limit = -1, offset = 0) {
  ...
  const words = await getUserDB().all(
    `select * from words
...
async function importCSV(id, plan) {
  ...
  for (let { word, paraphrase } of parseCSV(importFields, data)) {
    if (!word) continue;
    if (!paraphrase) {
      const res = await getDictDB().get(
        `select ${field} as paraphrase
...
const planInitializers = {
  async library(id, plan) {
    if (plan.dict === "rich") {
      // Query rich_dict.db: words from selected book + first translation as paraphrase
      const rdb = getRichDictDB();
      if (!rdb) throw new Error("Rich dictionary not available");
      const words = await rdb.all(
        `SELECT wm.word,
                (SELECT trans_cn FROM word_translations WHERE word_slug = wm.slug LIMIT 1) as paraphrase,
                wm.us_pronounce as phonetic
         FROM word_master wm
         WHERE wm.book_slug = ?
         ORDER BY wm.word`,
        plan.tag // plan.tag = book_slug
      );
      const now = Date.now();
      for (let i = 0; i < words.length; i++) {
        const { word, paraphrase, phonetic } = words[i];
        await getUserDB().run(
          `insert into words
           (plan_id, word, time, paraphrase, version) values (?, ?, ?, ?, ?)`,
          id,
    ...
    const words = await getDictDB().all(
      `
            ...
            from ${dict.table} where tag like ? order by ${plan.order}`,
      tag
    );
...
async function newPlan(_, plan) {
  ...
  await getUserDB().run(
    `insert into plans (id, name, version) values (?, ?, ?)`,
    planID,
    plan.name,
    Date.now()
  );
...
async function renamePlan(_, id, name) {
  await getUserDB().run(
    `update plans set name = ?, version = ? where id = ?`,
    name,
    Date.now(),
    id
  );
...
async function delPlan(_, id) {
  await getUserDB().run(
    `update plans set
        deleted = true, version = ? where id = ?`,
    Date.now(),
    id
  );
  await getUserDB().run(`delete from words where plan_id = ?`, id);
  if ((await getCurrentPlan()) === id) {
    const newPlan = await getUserDB().get(
      `select id from plans where not deleted limit 1`
    );
...
async function addWord(_, planID, word, time, paraphrase) {
  const w = await getUserDB().get(
    `select * from words
        where plan_id = ? and word = ? and not deleted`,
    planID,
    word
  );
  if (w) {
...
async function getWordList(_, tab, limit = -1, offset = 0) {
  ...
  const wordList = await getUserDB().all(
    `with u as (${sql}) select * from u limit ? offset ?`,
    ...args,
    limit,
    offset
  );
...
async function updateWord(_, planID, word, data) {
  ...
  // Track memorized_at timestamp
  if ("status" in data) {
    if (data.status === 1) {
      data.memorized_at = Date.now();
    } else if (data.status === 0) {
      data.memorized_at = null;
    }
  }
  ...
  if (data.word && word !== data.word) {
    ...
    await getUserDB().run(
      `delete from words
            where plan_id = ? and word = ? and deleted`,
      planID,
      data.word
    );

    // copy the word to the new name
    const st = await getUserDB().run(
      `insert or ignore into words
    ...
    if (st.changes === 0) {
      // copy failed, means the new word already exists
      return "duplicated-new-word";
    }

    // mark the old word as deleted
    await getUserDB().run(
      `update words set deleted = true, version = ?
            where plan_id = ? and word = ?`,
      data.version,
      planID,
      word
    );
  ...
  await getUserDB().run(
    `update words set ${fields.join(", ")} where word = ? and plan_id = ?`,
    ...values,
    word,
    planID
  );
...
async function getWordAnalysis(_, word) {
  const result = await analyzeWord(word);
  return result;
}

async function getWordRichData(_, word) {
  const result = await consultRichDictionary(word);
  return result;
}
...
async function getBookList() {
  ...
  if (!rdb) return [];
  return rdb.all(
    `SELECT wb.slug, wb.name, COUNT(wm.slug) as word_count
     FROM word_books wb
     LEFT JOIN word_master wm ON wm.book_slug = wb.slug
     GROUP BY wb.slug
     ORDER BY wb.name`
  );
}
...
async function search(_, word) {
  ...
  const res = await getUserDB().get(
    `select * from words
        where word = ? and plan_id = ? and not deleted`,
    word,
    planID
  );
...
async function getWordsByPrefix(_, prefix) {
  ...
  const res = await getDictDB().all(
    `select word from ${dict.table} where word like ? limit 100`,
    `${prefix}%`
  );
...
async function showWordDetail(_, word) {
  const mainWin = getMainWin();
  if (mainWin) {
    mainWin.show();
    mainWin.webContents.send("showDetail", word);
  }
}
...
async function importPlan() {
  const file = await dialog.showOpenDialog(getMainWin(), {
    ...
    title: "Import plan",
    properties: ["openFile"],
    filters: [{ name: "Plans", extensions: ["csv", "json"] }],
  });
...
async function showAbout() {
  ...
  await about.loadFile("renderer/about.html", {
...
async function exportPlan(_, id) {
  ...
  const file = await dialog.showSaveDialog(getMainWin(), {
    defaultPath: app.getPath("home"),
    title: "Export plan",
    filters: [{ name: "Plans", extensions: ["json"] }],
  });
  if (file.filePath) {
...
async function openDisplayArea() {
  ...
  await displayArea.loadFile("renderer/displayArea.html", {
